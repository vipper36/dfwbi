...
//------------------------------------------------------------------------
//  SetCtrs - set center points by layer.
//------------------------------------------------------------------------

void KMdata::SetCtrs(			// set center points
    KMcenterArray	sample,			// where to store centers
    int			k)			// number of points to sample
{
    
    for (int i = 1; i <= k; i++) {		// sample each point of sample
	 int ri=int(pow(i,i*log(nPts)/(k*log(k))));
	 kmCopyPt(dim, pts[ri-1], sample[i-1]);	// copy sample point
