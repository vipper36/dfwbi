...
void BlockCluster::analyze_common() {
    ...
    it++;
    for (; it != blocks.end(); it++) {
        common_node = find_common2(common_node, it->GetBlockList());
    }
...
void BlockCluster::analyze_main_section() {
    ...
    //    LOG << "common path size "<<common_node_path.size()<<"\n";
    main_node.clear();
...
void BlockCluster::analyze_date_author_source() {
    ...
    for (it = all_valid_node.begin(); it != all_valid_node.end(); it++) {
        ...
        int count = it->second;
        //LOG << "valid path:" << path << "\n";
//        if (path.find("STRONG") != string::npos) {
//            LOG << "valid path:" << path << "\n";
//        }
        ...
        list<BlockList>::iterator it_bl;
        int cluster_size = blocks.size();
        for (it_bl = blocks.begin(); it_bl != blocks.end(); it_bl++) {
            string text = it_bl->GetPathText(path);
            int w = text.length();
            if (w <= 0 && w > 200) continue;

            float num = count / (float)cluster_size;
//            if (path.find("STRONG") != string::npos) {
//                LOG << path << " " << text << " count: " << num << "\n";
//            }
            if (num < 3) {
...
float BlockCluster::is_date_node(string text) {
    ...
    if (!b && text.size() < 200) {
        if( S < 0.5){
            float p_date = all_words_relate_date(text);
            if(!b && p_date > 0.8){
                return p_date;
            }
        }
