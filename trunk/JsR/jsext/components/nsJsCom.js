...
var rCom = Components.classes["@com.xiao.xpcom/nsIRCom;1"]
                            .getService(Components.interfaces.nsIRCom);
...
{
    var uwin=XPCNativeWrapper.unwrap(win);
    var sandbox=new Components.utils.Sandbox(win);
    sandbox.alert=function(msg){
	win.alert(msg);
    }
    sandbox.R=function(expr){
	return rCom.eval(expr);
    }
    sandbox.window = win;
    if(!uwin.jQuery)
    {
	Services.scriptloader.loadSubScript("chrome://jsext/content/jquery.js",
				   win, "UTF-8");
    }
    if(uwin.$)
    {
    ...
    Components.utils.evalInSandbox("var tmpfun=function(){"+script+"};tmpfun()", sandbox);
}
