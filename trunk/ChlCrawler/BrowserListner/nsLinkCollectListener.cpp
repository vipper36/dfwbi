#include "nsLinkCollectListener.h"
...
NS_GENERIC_FACTORY_CONSTRUCTOR(nsLinkCollectListener)

static nsModuleComponentInfo components[] =
{
     {
	  NS_LINKCOLLECTLISTENER_CLASSNAME, 
	  NS_LINKCOLLECTLISTENER_CID,
	  NS_LINKCOLLECTLISTENER_CONTRACTID,
	  nsLinkCollectListenerConstructor,
     }
};
NS_IMPL_NSGETMODULE("nsLinkCollectListenerModule", components)

nsLinkCollectListener::nsLinkCollectListener()
:nsBrowserListener(),
     bodyCheckCount(0)
{
     nsresult rv;
}
NS_IMETHODIMP nsLinkCollectListener::Update(PRUint16 stat)
{
    if(mUrl!=nsnull)
        urlSrc->UpdateStat(mUrl,stat);
     return NS_OK;
}
NS_IMETHODIMP nsLinkCollectListener::GetUrl(nsACString &url)
{
...
NS_IMETHODIMP nsLinkCollectListener::SetWorkSpace(const nsACString & workspace)
{
	...
	nsString n;
	analyzer=do_CreateInstance("@nyapc.com/XPCOM/nsDOMCollector;1", &rv);
	if (NS_FAILED(rv))
	{
...
NS_IMETHODIMP nsLinkCollectListener::Excute(nsIWebNavigation *nav)
{
     nsresult rv=NS_OK;
