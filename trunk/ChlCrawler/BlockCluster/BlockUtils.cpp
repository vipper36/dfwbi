...
void convert_html_to_bl(nsIDOMHTMLDocument * html, BlockList & bl) {
    if (html != nsnull) {
        /*nsCString path;
        rv = bp->GetXPath(path);
        bl.SetPath(string(path.get()));
        nsCString url;
        rv = bp->GetDocUrl(url);
        bl.SetUrl(string(url.get()));
        nsCString title;
        rv = bp->GetDocTitle(title);
        bl.SetTitle(string(title.get()));
        //LOG<<"block title "<<bl.GetTitle()<<"\n";
		
        PRInt32 len;
        rv = bp->GetLinksLength(&len);
        for(PRInt32 i=0; i<len; i++)
        {
                nsCString l;
                bp->GetLinkByIndex(i,l);
                bl.add_link(string(l.get()));
                //LOG << "insert link "<<l.get()<<"\n";
        }*/

        nsCOMPtr<nsIDOMNode> node = do_QueryInterface(html);
        nsCOMPtr<nsIDOMNSDocument> nsdoc = do_QueryInterface(html);
        parse_doc(node, bl, nsdoc);
    }

}

void parse_doc(nsIDOMNode * node, BlockList & bl, nsIDOMNSDocument * html) {
    if (node == nsnull) {
    ...
    if (has_child) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (node_name.at(0) != '#') {
                ...
                path = get_node_path(n, path);
                path.append(bl.GetPath());
                //LOG << "### path..." << path <<"\n";
                b.SetPath(path);
                bool allowed = true;
                allowed = node_name.compare("OPTION") != 0;
...
void get_content(nsIDOMNode * node,
        nsCString & content,
    ...
    if (has_child) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (record && type == 3) {
                nsString value;
                n->GetNodeValue(value);
                content.Append(NS_ConvertUTF16toUTF8(value));
            } else {
                get_content(n, content, collection);
            }
...
string GetNodeText(nsIDOMNode * node) {
    ...
    return string(NS_ConvertUTF16toUTF8(Value).get());
     */
